...
public AdminController(DbHelper db, EmailService email, IConfiguration config)
{
    ...
    _config = config;
        ...
        public IActionResult Dashboard()
        {
            ...
            // Campanita en dashboard tambiÃ©n
        ...
        // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
        // ACEPTAR SOLICITUD â†’ descuenta stock + asigna repuesto a la orden
        // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
        [HttpPost]
        [ValidateAntiForgeryToken]
            ...
            string repuestoNombre, int cantidad)
        {
            ...
            if (idRepuesto == 0)
            {
                TempData["Error"] = $"No se encontrÃ³ '{repuestoNombre}' en inventario. Verifica el nombre.";
                ...
                WHERE id_repuesto = @idRep AND stock_actual >= @cantidad", conn))
            {
                ...
                if (filas == 0)
                {
                    TempData["Error"] = "No se pudo descontar el stock. IntÃ©ntalo de nuevo.";
                    return RedirectToAction("Inventario");
                }
            }
            ...
            // 6. Registrar en EstadosOrden para que el mecÃ¡nico lo vea en mensajes
            RegistrarEstado(conn, ordenId, idAdmin,
                "En proceso",
                $"Repuesto '{repuestoNombre}' Ã— {cantidad} asignado a esta orden por el administrador.");

            TempData["Exito"] = $"Repuesto '{repuestoNombre}' Ã— {cantidad} asignado a OT-{ordenId}. Stock actualizado.";
            return RedirectToAction("Inventario");
        }
        ...
        public IActionResult RechazarSolicitudRepuesto(int solicitudId)
        {
            ...
            // Marcar como atendida (rechazada = atendida tambiÃ©n para limpiar la lista)
            ...
            // Notificar al mecÃ¡nico via EstadosOrden
            if (idOrden > 0)
            {
            ...
            TempData["Exito"] = "Solicitud rechazada. El mecÃ¡nico fue notificado.";
            return RedirectToAction("Inventario");
        }
        ...
        // POST: Crear orden  â† AQUÃ va el correo
        [HttpPost]
        [ValidateAntiForgeryToken]
            ...
            string tipoServicio, string descripcionProblema, int kmIngreso)
        {
            ...
            // â”€â”€ Enviar correo al cliente â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        // POST: Cambiar estado de orden  â† AQUÃ tambiÃ©n va correo de estado
        [HttpPost]
        [ValidateAntiForgeryToken]
            ...
            string? observacion, int? idMecanico)
        {
            ...
            // â”€â”€ Enviar correo al cliente sobre cambio de estado â”€â”€â”€â”€â”€â”€â”€
            string correoCliente = "";
            string nombreCliente = "";
            string placa = "";
        ...
        // POST: Crear factura desde orden  â† AQUÃ tambiÃ©n va correo de factura
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> CrearFactura(int idOrden, decimal subtotal, decimal iva)
        {
            ...
            // â”€â”€ Enviar correo de factura al cliente â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            string correoCliente = "";
            string nombreCliente = "";
            string placa = "";
